...
def generate_ascii_name(state, recurrence, duration):
    state = state.lower()
    return "na2_"+state+"_"+str(recurrence)+"yr"+str(duration)+"hr.asc"

def read_ascii_grid(state, recurrence, duration, lat, long:)
    # get ascii file name
    ...
    # pull ascii file
    try
        print(joinpath(script_dir, ascii_name))
    ...
    print("$recurrence year $duration hour depth: $val_bilinear")
    return (state=state,
...
"""
"""
def generate_one_hour_data(six_hour_data, twentyfour_hour_data, region::Int, lat::Float64, long::Float64, elev::Float64)
...
"""
def compute_two_hour_depths(one_hours, six_hours, region::Int)
    if region === 1
...
def compute_three_hour_depths(one_hours, six_hours, region::Int)
    if region === 1
...
def generate_full_data(state::String, region::Int, lat::Float64, long::Float64, elev::Float64):
